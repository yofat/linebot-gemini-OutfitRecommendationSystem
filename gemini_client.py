...
def translate_to_japanese_keywords(chinese_suggestions: list) -> list:
    ...
    try:
        ...
        prompt = (
            f"將以下中文服飾名稱翻譯成日文購物關鍵字(用於日本樂天):\n\n"
            f"{items_text}\n\n"
            f"翻譯規則:\n"
            f"1. 只翻譯服飾名稱,保持簡潔,不要加形容詞\n"
            f"2. 顏色(片假名): 白色→ホワイト、黑色→ブラック、藍色→ブルー、深藍色/午夜藍→ネイビー、紅色→レッド、灰色→グレー、棕色→ブラウン、米色/卡其→ベージュ、粉色→ピンク\n"
            f"3. 服飾:\n"
            f"   襯衫/上衣→シャツ\n"
            f"   西裝褲→スラックス\n"
            f"   長褲→パンツ\n"
            ...
            f"   T恤→Tシャツ\n"
            f"   毛衣→セーター\n"
            f"4. 格式: [顏色] [服飾類型] (例: ホワイト シャツ)\n"
            f"5. 每行一個翻譯,不要編號,不要其他說明文字\n\n"
            f"翻譯結果:"
        )
