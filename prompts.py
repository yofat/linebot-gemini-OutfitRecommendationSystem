...
TASK_INSTRUCTION = (
    ...
    "}\n"
    "說明：\n"
    "- `suggestions` 必須是服飾或鞋類商品詞組，禁止推薦包包、配件、飾品或其他非穿著品。請保持格式為「顏色/材質 + 服飾品項 + 版型/剪裁」，例如：\"白色 シャツ スリム\"、\"ネイビー スラックス テーパード\"。\n"
    "- 若可推斷出性別，請於 `gender` 填入；若性別不明，請填空字串或 `不公開`，並讓 `suggestions` 採用男女皆可穿的選項或同時標示 メンズ/レディース 版本。\n"
    "- `preferences` 僅保留與服飾材質、版型、風格相關且可支援搜尋的關鍵字。\n"
    "- 嚴禁輸出任何配件、包袋、飾品、彩妝、生活用品或超出穿著範疇的建議。\n"
