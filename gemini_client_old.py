...
def analyze_outfit_image(scene: str, purpose: str, time_weather: str,
                        ...
                        timeout: int = 15) -> dict:
    """
    Multimodal image->JSON analyzer using the new google-genai SDK Client API (Free-tier friendly).
    ...
    # combine to final prompt used by new genai.Client API
    prompt = base_task + "\n" + schema + "\n" + example + "\n" + context_text
    ...
    try:
        ...
        # New SDK uses simpler API: client.models.generate_content()
        ...
        try:
            ...
            if 'overall_score' not in result or 'summary' not in result:
                logger.warning('Gemini response missing required fields, using fallback')
        ...
        msg = str(e)
        msg_lower = msg.lower()
        logger.exception('Error calling Gemini API: %s', msg)
        
        # Handle model not found errors
        if ('not found' in msg_lower or 'does not exist' in msg_lower):
            logger.error('Model %s not available. Try: gemini-2.0-flash-exp or gemini-1.5-flash', model_name)
            return _fallback_outfit_json(f'Model {model_name} not available')
