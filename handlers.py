...
def register_handlers(line_bot_api: LineBotApi, handler):
    ...
    def on_postback(event):
        ...
        if data == 'action=shop':
            ...
            # Translate Chinese clothing suggestions to Japanese keywords for Rakuten API
            # DO NOT translate scene/purpose/time_weather - they cause translation errors
            # Focus only on clothing item names for better search results
            try:
            ...
            try:
                ...
                # Pass empty strings for scene/purpose/time_weather to avoid mixed language queries
                # Rakuten search works better with just clothing items + gender
                queries = build_queries(japanese_keywords, '', '', 
                                      time_weather='', gender=gender, preferences=preferences)
